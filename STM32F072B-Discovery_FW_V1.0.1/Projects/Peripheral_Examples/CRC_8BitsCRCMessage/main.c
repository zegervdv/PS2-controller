...
#define BUFFER_SIZE            (countof(aCRCBuffer) - 1)
/* Private macro -------------------------------------------------------------*/    
/* Private variables ---------------------------------------------------------*/
...
int main(void)
{
  uint8_t aCRCBuffer[] = "STM32F0xx CortexM0 Device running on STM32F072B-DISCOVERY";
  uint8_t bComputedCRC = 0;
  uint8_t ExpectedCRC = 0xBE; /* The expected CRC value of aCRCBuffer using the
  ...
  */ 
  ...
  /* Initialize LEDs available on STM32F072B-DISCO board */
  ...
  /* Compute the CRC value of the 8-bit buffer: aCRCBuffer */
  bComputedCRC = CRC_8BitsCompute(aCRCBuffer, BUFFER_SIZE);
  
  /* Check if the computed CRC matches the expected one */
  if (bComputedCRC != ExpectedCRC)
  {
    /* Turn on LD3 */
    STM_EVAL_LEDOn(LED3);
  }
